...
type ChatServiceInterface interface {
	SendMessage(conn *websocket.Conn, messageData []byte) error
}
...
func NewWsService(db *gorm.DB, logger *zap.Logger) *WsService {
	return &WsService{db: db, logger: logger}
}

// 发送消息
func (s *WsService) SendMessage(conn *websocket.Conn, messageData []byte) error {
	var msg dto.Message
...
// 添加连接
func (s *WsService) AddConnection(userID uint64, conn *websocket.Conn) {
	libs.Mu.Lock()
...
// 移除连接
func (s *WsService) RemoveConnection(userID uint64) {
	libs.Mu.Lock()
...
// 获取接收者的连接
func (s *WsService) GetReceiverConnection(receiverID uint64) *websocket.Conn {
	libs.Mu.Lock()
...
// 心跳检测
func (s *WsService) Heartbeat(conn *websocket.Conn, userID uint64) {
...
func (s *WsService) IsUserOnline(userID uint64) bool {
...
func (s *WsService) StoreOfflineMessage(msg dto.Message) error {
	...
	repo := chat.NewChatMsgRepository(s.db, s.logger)
